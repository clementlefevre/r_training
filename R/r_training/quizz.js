...
function start_quiz(quizz_data) {
  ...
  function checkAnswer(quizz_id) {
    ...
    if (user_answer == quizz_data[quizz_id].answer) {
  ...
  function create_question_block(quizz_id) {
    ...
    var question_data = quizz_data[quizz_id]
  ...
  parent_main.appendChild(score_elem)

}



var quizz_data = (function () {
  var json = null;
  ...
  $.ajax({
    ...
    'success': function (data) {
      quizz_data = data[0];
      console.log("quizz.json loaded.")
      start_quiz(quizz_data)
    },
    'error': function (data) {
      console.log(data)
      console.log("error by loading quizz.json")
      quizz_data = data[0];
      start_quiz(quizz_data)
    }
  });
